...
function initSatTracker() {
  ...
  function drawSatellite(s, angle, idx) {
    ...
    gr.addColorStop(0, '#fff');
    gr.addColorStop(0.4, '#4FC3F7');
    gr.addColorStop(1, '#1976D2');
  ...
  canvas.addEventListener('click', e => {
    ...
    if (hit !== null) {
      ...
      if (info) {
        info.innerHTML = `<h3>${s.emoji} ${s.name}</h3><p>${s.desc}</p><div class="modal-stat"><span>Орбита</span><strong>${s.orbit}</strong></div>`;
      }
    }
  });
...
function initBHSimulator() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    p.x = cx + Math.cos(angle) * SPAWN_RADIUS;
    p.y = cy + Math.sin(angle) * SPAWN_RADIUS;
  ...
  function draw() {
    ...
    if (!paused) {
      particles.forEach(p => {
        ...
        const force = (G * BH_RADIUS * 200) / (Math.max(dist, 10) * Math.max(dist, 10));
